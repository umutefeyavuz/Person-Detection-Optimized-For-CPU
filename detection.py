...
class Detector(object):
    def __init__(
            ...
            score_th=0.3,
            nms_th=0.5,
            providers=None, # OpenCV DNN providers tarafından yönetilir
            num_threads=None,
    ):
        ...
        if self.extension == 'onnx':
            # OpenCV DNN Kullan (onnxruntime bağımlılığını kaldırır)
            self.net = cv2.dnn.readNetFromONNX(model_path)
            # CPU üzerinde çalıştır (Optimized for CPU)
            self.net.setPreferableBackend(cv2.dnn.DNN_BACKEND_OPENCV)
            self.net.setPreferableTarget(cv2.dnn.DNN_TARGET_CPU)
        elif self.extension == 'tflite':
            # TFLite için tensorflow/tflite-runtime hala gerekli olabilir
            try:
                from tflite_runtime.interpreter import Interpreter
                self.model = Interpreter(model_path=model_path, num_threads=num_threads)
            except ImportError:
                import tensorflow as tf
                self.model = tf.lite.Interpreter(model_path=model_path, num_threads=num_threads)
    ...
    def inference(self, image):
        ...
        if self.extension == 'onnx':
            ...
            # image[None, :, :, :] zaten (1, 3, 192, 192) formatında prepocessten geliyor
            self.net.setInput(image[None, :, :, :])
